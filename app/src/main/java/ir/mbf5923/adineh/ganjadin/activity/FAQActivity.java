...
import ir.mbf5923.adineh.ganjadin.adapters.faq_adapter;
...
public class FAQActivity extends AppCompatActivity {
    ...
    private List<faq_model> model = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        adapter.setOnItemClickListener(new faq_adapter.ClickListener() {
            @Override
            public void onItemClick(int position, View v) {

                showanswerpopup(v, model.get(position).getAnswer());
    ...
    private void loadfaq() {
        ...
        NetworkManager.getInstance().loadfaq(new NetworkListener() {
            @Override
                ...
                try {
                    ...
                    if (result.getInt("id") == 1) {
                        ...
                        for (int i = 0; i < array.length(); i++) {
                            faq_model movie = new faq_model(array.getJSONObject(i).getString("question"), array.getJSONObject(i).getString("answer"));
                            model.add(movie);
                        }
                        adapter.notifyDataSetChanged();
                        new FancyShowCaseView.Builder(sp)
                                .focusOn(rvfaq)
                                .title("برای مشاهده جواب هر سوال بر وری آن سوال کلیک کنید")
